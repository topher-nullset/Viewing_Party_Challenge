    ...
    describe '#top_20_rated', :vcr do
      it 'returns the top 20 rated movies' do
        VCR.use_cassette('top_20_rated_movies', re_record_interval: 7.days) do
          top20 = MoviesService.new.top_20_rated

          expect(top20).to be_a Hash
          expect(top20[:results]).to be_an Array
          expect(top20[:results].length).to eq(20)

          number1 = top20[:results].first
